...
def index(request):
    ...
    shared_posts = SharedPost.objects.filter(shared_to=request.user).select_related('original_post', 'shared_by').order_by('-shared_at')
    context = {
        'posts': posts,
...
def share_post(request, post_id):
    ...
    if request.method == 'POST':
        ...
        # Save shared post
        SharedPost.objects.create(original_post=post, shared_by=request.user, shared_to=to_user)
        messages.success(request, f"Post shared with {to_user.username}")
        return redirect('index')
